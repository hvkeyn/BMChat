...
/// Canonical BMChat invite link prefix (HTTP host on the BMChat VPS, path `/i`).
const IDELTACHAT_SCHEME: &str = "http://5.187.4.132/i#";
/// All invite link prefixes that BMChat is willing to decode. The first entry
/// is the canonical BMChat one; the rest are legacy/placeholder hosts kept so
/// QR codes generated by older builds and any upstream Delta Chat installs
/// keep working.
const IDELTACHAT_SCHEMES: &[&str] = &[
    "http://5.187.4.132/i#",
    "http://5.187.4.132/i/#",
    "https://5.187.4.132/i#",
    "https://5.187.4.132/i/#",
    "https://i.bmchat.example/#",
    "https://i.bmchat.example#",
    "https://i.delta.chat/#",
    "https://i.delta.chat#",
];
...
pub async fn check_qr(context: &Context, qr: &str) -> Result<Qr> {
    ...
    } else if let Some(prefix) = IDELTACHAT_SCHEMES.iter().find(|p| qr.starts_with(*p)) {
        decode_ideltachat(context, prefix, qr).await?
...
/// scheme: `http://5.187.4.132/i#FINGERPRINT&a=ADDR[&OPTIONAL_PARAMS]`
/// (legacy: `https://i.bmchat.example[/]#...`, `https://i.delta.chat[/]#...`)
