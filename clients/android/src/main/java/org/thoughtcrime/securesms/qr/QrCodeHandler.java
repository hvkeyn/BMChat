...
public class QrCodeHandler {
  ...
  public void handleQrData(
    ...
    // BMChat 2.49.97: short-circuit the helper QR generated by the desktop
    // "Add as Second Device" dialog. If the user happens to scan it through
    // BMChat's own scanner instead of the system camera, route them straight
    // to the SENDER_SHOW_QR screen instead of falling back to the generic
    // "QR contains text" dialog.
    if (handleBmchatSecondDeviceDeepLink(rawString)) return;
  ...
  /**
   * BMChat 2.49.97: route the helper QR generated by the desktop "Add as
   * Second Device" dialog to {@code SecondDeviceDeepLinkActivity}, which
   * brings up SENDER_SHOW_QR directly without forcing the user through the
   * settings menu.
   *
   * @return true if the raw string was recognised as the BMChat helper
   *     deep-link and an intent was dispatched; false otherwise.
   */
  private boolean handleBmchatSecondDeviceDeepLink(String rawString) {
    if (rawString == null) return false;
    String trimmed = rawString.trim();
    if (trimmed.isEmpty()) return false;
    String lower = trimmed.toLowerCase();
    if (!lower.equals("bmchat://second-device")
        && !lower.equals("bmchat:/second-device")
        && !lower.startsWith("bmchat://second-device?")
        && !lower.startsWith("bmchat:/second-device?")) {
      return false;
    }
    Intent intent =
        new Intent(
            activity,
            org.thoughtcrime.securesms.bmchat.SecondDeviceDeepLinkActivity.class);
