...
APK=BMChat-foss-debug-2.50.16.apk
...
if [ -f "${DESK_LOCAL}/BMChat-2.50.16-Setup.x64.exe" ]; then
  scp -i "$KEY" -o StrictHostKeyChecking=no \
    "${DESK_LOCAL}/BMChat-2.50.16-Setup.x64.exe" \
    "${DESK_LOCAL}/BMChat-2.50.16-Portable.x64.exe" \
    "$HOST:/tmp/"
  ssh -i "$KEY" -o StrictHostKeyChecking=no "$HOST" bash -se <<'MEOF'
set -euo pipefail
WEBROOT=/var/www/bmchat
sudo install -m 0644 /tmp/BMChat-2.50.16-Setup.x64.exe    "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/BMChat-2.50.16-Portable.x64.exe "$WEBROOT/desktop/"
cd "$WEBROOT/desktop"
sudo ln -sfn BMChat-2.50.16-Setup.x64.exe    BMChat-Setup-x64.exe
sudo ln -sfn BMChat-2.50.16-Portable.x64.exe BMChat-portable-x64.exe
...
MEOF
  echo "[mirror] desktop 2.50.16 Win uploaded"
fi
