#!/usr/bin/env bash
# BMChat Desktop 2.50.16 — primary VPS uploader (Win + manifest).
set -euo pipefail
...
SETUP="BMChat-2.50.16-Setup.x64.exe"
PORTABLE="BMChat-2.50.16-Portable.x64.exe"
...
install -m 0644 /tmp/bmchat-desktop/BMChat-2.50.16-Setup.x64.exe    "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/BMChat-2.50.16-Portable.x64.exe "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/desktop-update.json            "$WEBROOT/desktop-update.json"
ln -sfn BMChat-2.50.16-Setup.x64.exe    "$WEBROOT/desktop/BMChat-Setup-x64.exe"
ln -sfn BMChat-2.50.16-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
...
echo "[uploader-primary-desktop] 2.50.16 done."
EOF
