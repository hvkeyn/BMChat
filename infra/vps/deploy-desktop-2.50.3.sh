#!/usr/bin/env bash
# BMChat Desktop 2.50.3 — primary VPS uploader (Win + Linux + manifest).
set -euo pipefail
...
install -m 0644 /tmp/bmchat-desktop/BMChat-2.50.3-Setup.x64.exe    "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/BMChat-2.50.3-Portable.x64.exe "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/BMChat-2.50.3-x86_64.AppImage  "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/bmchat-desktop_2.50.3_amd64.deb "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/desktop-update.json            "$WEBROOT/desktop-update.json"

ln -sfn BMChat-2.50.3-Setup.x64.exe    "$WEBROOT/desktop/BMChat-Setup-x64.exe"
ln -sfn BMChat-2.50.3-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
ln -sfn BMChat-2.50.3-x86_64.AppImage  "$WEBROOT/desktop/BMChat-x86_64.AppImage"
ln -sfn bmchat-desktop_2.50.3_amd64.deb "$WEBROOT/desktop/bmchat-amd64.deb"
