...
SETUP="BMChat-2.50.37-Setup.x64.exe"
PORTABLE="BMChat-2.50.37-Portable.x64.exe"
...
sudo install -m 0644 /tmp/bmchat-desktop-mirror/BMChat-2.50.37-Setup.x64.exe "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/bmchat-desktop-mirror/BMChat-2.50.37-Portable.x64.exe "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/bmchat-desktop-mirror/desktop-update.json "$WEBROOT/desktop-update.json"
sudo ln -sfn BMChat-2.50.37-Setup.x64.exe "$WEBROOT/desktop/BMChat-Setup-x64.exe"
sudo ln -sfn BMChat-2.50.37-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
sudo chown -R www-data:www-data "$WEBROOT/desktop" "$WEBROOT/desktop-update.json"
echo "[uploader-mirror-desktop] 2.50.37 done."
EOF