#!/usr/bin/env bash
# BMChat Desktop 2.50.3 — mirror VPS uploader (Yandex Cloud, dante user). Win + Linux + manifest.
set -euo pipefail
...
SETUP="BMChat-2.50.3-Setup.x64.exe"
PORTABLE="BMChat-2.50.3-Portable.x64.exe"
APPIMAGE="BMChat-2.50.3-x86_64.AppImage"
DEB="bmchat-desktop_2.50.3_amd64.deb"
...
sudo install -m 0644 /tmp/BMChat-2.50.3-Setup.x64.exe    "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/BMChat-2.50.3-Portable.x64.exe "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/BMChat-2.50.3-x86_64.AppImage  "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/bmchat-desktop_2.50.3_amd64.deb "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/desktop-update.json            "$WEBROOT/desktop-update.json"

sudo ln -sfn BMChat-2.50.3-Setup.x64.exe    "$WEBROOT/desktop/BMChat-Setup-x64.exe"
sudo ln -sfn BMChat-2.50.3-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
sudo ln -sfn BMChat-2.50.3-x86_64.AppImage  "$WEBROOT/desktop/BMChat-x86_64.AppImage"
sudo ln -sfn bmchat-desktop_2.50.3_amd64.deb "$WEBROOT/desktop/bmchat-amd64.deb"
