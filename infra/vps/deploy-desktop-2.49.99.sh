#!/usr/bin/env bash
# BMChat Desktop 2.49.99 — primary VPS uploader.
set -euo pipefail
...
install -m 0644 /tmp/bmchat-desktop/BMChat-2.49.99-Setup.x64.exe    "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/BMChat-2.49.99-Portable.x64.exe "$WEBROOT/desktop/"
install -m 0644 /tmp/bmchat-desktop/desktop-update.json             "$WEBROOT/desktop-update.json"

ln -sfn BMChat-2.49.99-Setup.x64.exe    "$WEBROOT/desktop/BMChat-Setup-x64.exe"
ln -sfn BMChat-2.49.99-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
