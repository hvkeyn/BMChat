#!/usr/bin/env bash
# BMChat Desktop 2.49.99 — mirror VPS uploader (Yandex Cloud, dante user).
set -euo pipefail
...
SETUP="BMChat-2.49.99-Setup.x64.exe"
PORTABLE="BMChat-2.49.99-Portable.x64.exe"
...
sudo install -m 0644 /tmp/BMChat-2.49.99-Setup.x64.exe    "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/BMChat-2.49.99-Portable.x64.exe "$WEBROOT/desktop/"
sudo install -m 0644 /tmp/desktop-update.json             "$WEBROOT/desktop-update.json"

sudo ln -sfn BMChat-2.49.99-Setup.x64.exe    "$WEBROOT/desktop/BMChat-Setup-x64.exe"
sudo ln -sfn BMChat-2.49.99-Portable.x64.exe "$WEBROOT/desktop/BMChat-portable-x64.exe"
