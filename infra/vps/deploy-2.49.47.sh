#!/usr/bin/env bash
# BMChat 2.49.47 — Email-боты. Заливает новый APK + update.json на
# primary (5.187.4.132) и поднимает символическую ссылку latest.apk.
set -euo pipefail
...
APK_NAME="BMChat-foss-debug-2.49.47.apk"
...
${SSH} "curl -sS http://127.0.0.1/update.json | head -16"
${SSH} "curl -sS -I http://127.0.0.1/apk/${APK_NAME} | head -4"
